...
@click.option("--with-sample", is_flag=True, default=False,
              help="Also copy the filled sample.xlsx for reference (only available in source checkouts).")
@click.option("-f", "--force", is_flag=True, default=False,
...
def init(path, with_sample, force):
    ...
    # Blank xlsx — generated at runtime so the package needs no .xlsx asset.
    cases_path = target / "cases.xlsx"
    if cases_path.exists() and not force:
        _log(f"skip {cases_path} (already exists, use -f to overwrite)")
    else:
        T.write_blank_xlsx(cases_path)
        _log(f"wrote {cases_path}")
        written += 1

    # YAMLs — bundled as Python strings.
    text_files = [
        ("phases.yaml", T.PHASES_YAML),
        ("glossary.yaml", T.GLOSSARY_YAML),
        ("README.md", T.INIT_README),
    ]
    ...
    # Optional sample copy (only when a source checkout is present).
    if with_sample:
        pkg_root = Path(__file__).parent.parent
        src = pkg_root / "examples" / "sample.xlsx"
        if src.exists():
            dst = target / "sample.xlsx"
            if dst.exists() and not force:
                _log(f"skip {dst} (already exists, use -f to overwrite)")
            else:
                shutil.copy(src, dst)
            ...
            _log("WARN: --with-sample requested but examples/sample.xlsx not found; skipping")
